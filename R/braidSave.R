...
#' @method braidSave default
#' @export
braidSave.default <- function(b){
  text <- braidAppendText(b)
  sfile <- file(b$fileInner, "at")  ### Open file in append mode
  on.exit(close(sfile))
  cat(text, file=sfile, append=TRUE)
  braidAppendText(b, reset=TRUE)
